...
export function normalizeMode(mode) {
  ...
  if (!KIMI_MODES.includes(normalized)) {
    throw new Error(`unsupported Kimi mode: ${mode}`);
  }
  return normalized;
}
...
You are Kimi K2.6 working directly with Codex through the local kci wrapper.
...
- Do not act as Reasonix engineering final review. Engineering final review stays with crb/codex-reasonix.
${modeSpecific}
...
export function buildUserPrompt({ task, contexts = [], files = [], images = [], imageHandling = "inline" }) {
  const parts = [];
  parts.push("Task:");
  ...
  if (images.length > 0) {
    ...
    if (imageHandling === "kimi-code-read-media") {
      parts.push(
        "Kimi Code media handling: before answering, call ReadMediaFile for every image path above. " +
        "If any image cannot be read, state [IMAGE_NOT_READ:<path>] and do not perform visual critique for that image.",
      );
    } else {
