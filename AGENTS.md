...
- For screenshots, use `kci delegate --mode ui-review-cn --json --image <path> "<task>"`. Do not say Kimi saw an image unless `image_payload_sent: true` is present and the command succeeded.
...
kci delegate --mode ui-review-cn --json --image /tmp/red.png "识别主色，只返回 RED"
